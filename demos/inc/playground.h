...
EVAL(REPEAT(3, A, T))
// template< class> class T0 , template< class , class> class T1 , template< class , class , class>
// class T2
...
#define WHILE_DO(pred, op, ...) EVAL(WHILE(pred, op, __VA_ARGS__))
WHILE_DO(PRED, OP, 8)

//
//
//
#define IS_EMPTY(x, ...) CHECK(CAT(PRIMITIVE_CAT(IS_EMPTY_, x), TRUE))
#define IS_EMPTY_TRUE    PROBE()
IS_EMPTY()

#define FOR_EACH_A0(x, ...) CAT(FOR_EACH_, IS_EMPTY(__VA_ARGS__))
#define FOR_EACH_A1(x, ...) (x, __VA_ARGS__)

// #define FOR_EACH(macro, x, ...) CAT(FOR_EACH_, IS_EMPTY(__VA_ARGS__)) (macro, x, __VA_ARGS__)
#define FOR_EACH(macro, ...)      FOR_EACH_A0(__VA_ARGS__) FOR_EACH_A1(macro, __VA_ARGS__)
...
#define MY_SEQ() VFUNC, a, b, c, VFUNC, d, e, VFUNC, f

EVAL(FOR_EACH(FOO, MY_SEQ()))
...
#define MYFACE VFUNC(a, b, c), VFUNC(d, e, g), VFUNC(f, dd, b),
MYFACE

#define GENIMPL(x) EXPAND(CAT(GENIMPL_, x))

#define MYFACE_EMMT
IS_EMPTY(MYFACE_EMMT)

// #define CONSTRUCT_MACRO(tag, ...) tag, __VA_ARGS__)
#define GENIMPL_VFUNC(_trait, struct, func, ...) .func = struct##_##func,

EVAL(FOR_EACH(GENIMPL, MYFACE))
